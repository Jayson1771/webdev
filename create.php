...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!isset($_SESSION['id'])) {
        die("<script>alert('You must be logged in to access this page.'); window.location.href='login.php';</script>");
    }

    $student_id = $_SESSION['username']; // Logged-in user ID
    $height = $_POST['height'];
    $weight = $_POST['weight'];
    $age = $_POST['age'];
    $sex = $_POST['sex'];
    ...
    $query = "INSERT INTO studentdata 
              (LRN_number, name, weight, height, age, sex, section, medical_history) 
              VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    if (mysqli_stmt_execute($stmt)) {
        echo "<script>alert('Medical assessment submitted successfully!'); window.location.href='home.php';</script>";
    } else {
